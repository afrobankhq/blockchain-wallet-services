...
const createMasterWallet = async (req, res) => {
    try {
        ...
        const masterWallet = await firebaseService_1.masterWalletService.create({ name, metadata });
        res.json({
            success: true,
            data: {
                id: masterWallet.id,
...
const createDedicatedAddress = async (req, res) => {
    try {
        ...
        const masterWallet = await firebaseService_1.masterWalletService.findById(master_wallet_id);
        if (!masterWallet) {
            return res.status(404).json({ error: 'Master wallet not found' });
        }
        const dedicatedAddress = await firebaseService_1.dedicatedAddressService.create({
            customer_id,
            master_wallet_id,
            name,
            disable_auto_sweep,
